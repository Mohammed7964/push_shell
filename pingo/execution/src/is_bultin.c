...
/*   Updated: 2025/06/18 04:04:33 by omaezzem         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_builtins(t_ctx *ctx, t_cmd *data)
{
	if (data->files && data->redirection && notpipe(data))
		ft_do_redirections(data->files, data->redirection, ctx->fdher, data);
	if (ft_strcmp(data->cmd[0], "echo") == 0)
		return ((data->ex_status = ft_echo(&data->cmd[0])), 1);
	else if (ft_strcmp(data->cmd[0], "cd") == 0)
		return ((data->ex_status = ft_cd(*ctx->env, data->cmd, *ctx->exp)), 1);
	else if (ft_strcmp(data->cmd[0], "pwd") == 0)
		return (data->ex_status = ft_pwd(*ctx->env), 1);
	else if (ft_strcmp(data->cmd[0], "export") == 0)
		return (data->ex_status = ft_export(*ctx->exp,
				*ctx->env, data->cmd), 1);
	else if (ft_strcmp(data->cmd[0], "env") == 0)
		return (data->ex_status = builtin_env(ctx->env, data->cmd), 1);
	else if (ft_strcmp(data->cmd[0], "exit") == 0)
		return (ft_exit(data, data->cmd, ctx), 1);
	else if (ft_strcmp(data->cmd[0], "unset") == 0)
		return (data->ex_status = ft_unset(ctx->exp, ctx->env, data->cmd), 1);
	else
		return (0);
}
