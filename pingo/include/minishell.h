...
/*   Updated: 2025/06/18 22:31:13 by mel-badd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
# define SUCCESS 1
# define PMAX 4096
# define PATH_ENV "/mnt/homes/omaezzem/brew/bin:/mnt/homes/omaezzem/brew/bin:\
/mnt/homes/omaezzem/brew/bin:/mnt/homes/omaezzem/goinfre/homebrew/bin:\
...
int			ft_execute(t_ctx *ctx, t_cmd **data, char **envp,
				t_heredoc *heredoc);
...
int			process_redirection_in(char *file, char *redir, int heredoc_fd,
				int *in_fd);
...
void		add_to_export_list_vl(t_exp **exp, char *avzero,
				char *avone);
...
void		add_to_export_list_vl(t_exp **exp, char *avzero,
				char *avone);
void		add_to_export_list_v(t_exp **exp, char *avzero);
void		add_to_env_list(t_env *env, char *args, char *avzero,
				char *avone);
...
int			ft_builtins(t_ctx *ctx, t_cmd *data);
...
int			is_cmd_path(t_cmd *curr, char **commande,
				t_ctx *ctx, int i);
int			join_execve_cmd(char *path, char **commande,
				t_cmd *curr, t_ctx *ctx);
int			ft_execve(t_cmd *curr, t_env **env, t_ctx *ctx, int i);
void		free_pipes(int (*pipes)[2], t_cmd *data, int *pids,
				int numcmd);
int			create_pipes(int numcmd, int (*pipes)[2], int *pids,
				t_cmd *data);
...
void		determine_export_case(t_exp *exp, t_env *env, char *arg,
				char *avzero);
void		handle_equal_case(t_exp *exp, t_env *env, char *arg,
				char *avzero);
void		handle_plus_case(t_exp *exp, t_env *env, char *arg,
				char *avzero);
...
void		kill_pro(t_ctx *ctx, int numcmd);
...
char		*append_to1_result1(t_gc *gc, char *result, char *str,
				size_t *total);
void		handle_expansion(t_ctx *ctx, t_token **cur, t_token *tk,
				t_cmd *cmd);
void		clear_fd(void);
...
int			tokenize_next_token(t_gc *gc, char *input, t_token **head,
				int i);
char		*dyn_str_finalize(t_gc *gc, t_dynstr *ds);
int			dyn_str_append(t_gc *gc, t_dynstr *ds, const char *str,
				size_t len);
