...
/*   Updated: 2025/06/18 04:05:16 by omaezzem         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_execute_pipes(t_ctx *ctx)
{
	t_cmd	*curr;
	int		i;
	...
	while (++i < ctx->numcmd && curr)
	{
		ctx->pids[i] = fork();
		if (ctx->pids[i] == -1)
			return (kill_pro(ctx, ctx->numcmd), perror("fork"));
		if (ctx->pids[i] == 0)
